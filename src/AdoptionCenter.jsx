...
const ANIMALS = [
  ...
  { id: 'dragon', label: 'Dragon', Component: PetDragon },
]
...
const THEMES = [
  ...
  { id: 'ember', label: 'Dragon Ember', bg: 'from-orange-100 via-amber-100 to-yellow-100',    accent: '#f97316', border: 'border-orange-300' },
]
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <form
        ...
        className={`
          w-full max-w-sm rounded-3xl bg-gradient-to-b ${theme.bg}
          border-4 ${theme.border} shadow-2xl flex flex-col items-center gap-6 p-6
      ...
      >
        ...
        <div className="w-full flex flex-col gap-2">
          ...
          <input
            ...
            className={`
              ...
              placeholder-purple-300 outline-none focus:ring-2 focus:ring-purple-300 transition
            `}
          />
        </div>
        ...
        <div className="w-full flex flex-col gap-2">
          ...
          <div className="grid grid-cols-4 gap-2 justify-items-center">
            {ANIMALS.map(({ id, label, Component }) => (
              <button key={id} type="button" onClick={() => setAnimal(id)}
                className={`
                  flex flex-col items-center gap-1 p-1.5 rounded-2xl border-2 transition-all
                  ${animal === id
              ...
              >
                ...
                <span className="font-pixel text-[7px] text-purple-700">{label}</span>
              </button>
            ))}
          </div>
        </div>
            ...
            {THEMES.map(t => (
              <button key={t.id} type="button" onClick={() => setThemeId(t.id)}
                className={`
                  ...
                  ${themeId === t.id
                    ? 'border-purple-500 bg-white/80 scale-105 shadow'
                    : 'border-transparent bg-white/40 hover:bg-white/60'}
                `}
              >
        ...
        <button
          type="submit"
          disabled={!canStart}
          className="w-full py-3 rounded-2xl font-pixel text-xs text-white shadow-lg transition-all duration-150 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
          style={{ background: canStart ? `linear-gradient(135deg, ${theme.accent}, #c084fc)` : '#d1d5db' }}
        >
          Let&apos;s Go! ✦
        </button>
